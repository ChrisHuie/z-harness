...
Swept sources: `hooks/bash_command_guard.py`, `hooks/guards/git_grep_engine_guard.py`, `hooks/guards/zsh_rev_modifier_guard.py`. No other repository file is mutated by this sweep, so the exclusions listed at the end enumerate symbols inside these sources only.
