...
and is not a claim that every exemption-shaped collection in the tree carries one. Kills
scored only by a moved check count are recorded per result and reported by the gate rather
than shown in this summary: whether an assertion fires can differ between hosts, and this
file is compared byte for byte against a CI re-measurement.
