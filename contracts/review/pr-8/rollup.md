...
`a78992061991054a7ffdee71a812b247fcc959e4`. The three guard sources are byte-identical to
`3442f524cfa95caabbaf6836d6a2528f42890581`, where the mutation figures were measured, so
those figures describe the code under review. Any commit after `a789920` on this branch adds
this document and changes no guard, tool or contract.
...
elements are held by fewer than two checks and 132 by none — for those, removing the element
...
text, and the second is the drift check that would have to redden. That count is held as a
shrink-only ceiling in `tools/ci-gate.py`, so it can fall and cannot rise without failing.
...
worktree: 10 suites / 0 failures on macOS 15 and on git 2.47.3 under linux/arm64. Eight
...
git checkout a78992061991054a7ffdee71a812b247fcc959e4
python3 tools/ci-gate.py                                    # 10 suites / 0 failures
python3 hooks/harness_check.py --selftest                   # 75
python3 hooks/guards/git_grep_engine_guard.py --selftest    # 571
python3 hooks/guards/zsh_rev_modifier_guard.py --selftest   # 239
python3 hooks/bash_command_guard.py --selftest              # 1088
```
