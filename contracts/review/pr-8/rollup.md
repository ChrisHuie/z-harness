...
the cross-platform caught/survived fact, the reason each kill was scored, and exact mutation
IDs. Surviving set-element mutations are explicit coverage debt; no site mutation may
survive, and the debt ceiling may fall but not rise.

Element mutations only remove members. Removal makes a collection that grants an exemption
stricter, so declared additions carry the opposite expectation: each must be caught, and a
survivor there is a live fail-open rather than debt. Recording the kill reason separates a
mutation an assertion detected from one scored only because the recorded check count moved.
A guard that counts once per element of the collection under mutation moves that count on
any removal, so those kills are decided by loop structure rather than by detection; they are
tallied separately and carry their own ceiling. The aggregator enforces that ceiling on the
fresh normalized receipt before projecting host-observed reasons out of the cross-platform
comparison.
