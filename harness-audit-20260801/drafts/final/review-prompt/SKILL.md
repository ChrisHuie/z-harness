---
name: review-prompt
description: Reviewing, critiquing, auditing, or linting a prompt, system prompt, SKILL.md, AGENTS.md, CLAUDE.md, or GEMINI.md — severity-ranked findings, read-only, never edits. Use also when asked why an agent misbehaves or a skill will not trigger, or before committing prompt changes. Not for authoring new ones (craft-prompt, craft-skill, craft-context-file), application code, or PR diffs.
...
**Do not misapply the critic to source it was not designed to judge.** This catalog grades prompts, skills and context files; application code, PR diffs, governance and experimental plans answer to a different rubric — name the mismatch and stop, rather than producing confident findings from the wrong one.

**A SKILL.md under review on a branch is the repo tree, not the installed copy** — what the loader serves is a point-in-time snapshot with a flattened layout and may be stale. Say which one you read (`craft-skill` carries the rule).
