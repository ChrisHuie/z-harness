verified: 2026-07-14 · sources: skill-catalog §2; policy-blocks/uncertainty; goal adjective: nuanced

# Axis: calibration — how uncertainty and trade-offs surface
...
Lint hooks: hedging density at C0 (over-hedging is the failure); unqualified absolutes at C3. Rubric hooks: hedges appear exactly where warranted (calibration = neither over- nor under-confident); trade-offs present for material choices at C2+.

Interactions: pairs with uncertainty block (interactive vs autonomous variants); review stance defaults C2 (findings note edge cases); plan stance defaults C2+ (alternatives are contractual).
